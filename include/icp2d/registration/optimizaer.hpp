...
struct GaussNewtonOptimizer {
  ...
  optimize(const TargetPointCloud &target, const SourcePointCloud &source,
    ...
    for (int i = 0; i < max_iterations && !result.converged; i++) {
      ...
      // Solve linear system
      const Eigen::Matrix<double, 3, 1> delta =
          (H + lambda * Eigen::Matrix<double, 3, 3>::Identity())
              .ldlt()
              .solve(-b);

      if (verbose) {
        std::cout << "iter=" << i << " e=" << e << " lambda=" << lambda
                  << " dt=" << delta.tail<2>().norm()
                  << " dr=" << delta.head<1>().norm() << std::endl;
      }

      result.converged       = criteria.converged(delta);
      result.T_target_source = result.T_target_source * se2_exp_isometry(delta);
      result.iterations      = i;
      result.H               = H;
      result.b               = b;
      result.error           = e;
  ...
  double lambda; ///< Damping factor (Increasing this makes optimization slow
                 ///< but stable)
};
...
struct LevenbergMarquardtOptimizer {
  LevenbergMarquardtOptimizer()
      : verbose(false), max_iterations(20), max_inner_iterations(10),
        init_lambda(1e-3), lambda_factor(10.0) {}
